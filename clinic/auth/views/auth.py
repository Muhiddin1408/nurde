...
#
# from api.auth.serializers.register import RegisterSerializer, RegisterDoctorSerializer, RegisterClinicSerializer
...
# from apps.clinic.models.clinic_user import ClinicUser
...
#
#         if phone.isdigit():
#             user = User.objects.filter(username='c' + phone).last()
#         else:
#             user = User.objects.filter(username='c' + phone).last()
#         if user is None:
...
#             user = User.objects.get(username='c' + phone)
...
#         user = User.objects.filter(username='c' + phone, is_staff=True).first()
#         if user and user.check_password(password):
#             doctor = ClinicUser.objects.filter(user=user).last()
#             if doctor:
#                 user.save()
#                 token = RefreshToken.for_user(user)
#                 result = {
...
#             if status:
#                 if User.objects.get(username='c' + user_data['email']).is_active:
#                     user = User.objects.get(username='c' + user_data['email'])
#                 else:
#                     User.objects.get(username='c' + user_data['email']).delete()
